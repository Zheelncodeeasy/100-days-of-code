from turtle import Turtle

STARTING_POSITIONS = [(0, 0), (-20, 0), (-40, 0)]
MOVE_DISTANCE = 20
UP = 90
DOWN = 270
LEFT = 180
RIGHT = 0

class Snake:
    def __init__(self):
        self.segments = []
        self.create_snake()
        self.head = self.segments[0]  # head of snake

    def create_snake(self):
        for position in STARTING_POSITIONS:
            self.add_segment(position)

    def add_segment(self, position):
        new_sq = Turtle("square")
        new_sq.color("white")
        new_sq.penup()
        new_sq.goto(position)
        self.segments.append(new_sq)

    def extend_segment(self):
        self.add_segment(self.segments[-1].position()) # extending segments of snake based on position

    def move(self):
        # linking the segments of snake so that they move together with head
        for seg_no in range(len(self.segments) - 1, 0, -1):
            new_x = self.segments[seg_no - 1].xcor()
            new_y = self.segments[seg_no - 1].ycor()
            self.segments[seg_no].goto(new_x, new_y)
        self.segments[0].forward(MOVE_DISTANCE)

    def up(self):
        if self.head.heading() != DOWN: # don't move up when head is down
            self.head.setheading(UP)

    def down(self):
        if self.head.heading() != UP: # don't move down when head is up
            self.head.setheading(DOWN)

    def left(self):
        if self.head.heading() != RIGHT: # don't move left when head is right
            self.head.setheading(LEFT)

    def right(self):
        if self.head.heading() != LEFT: # don't move right when head is left
            self.head.setheading(RIGHT)